-- Safe RPC: returns only names/avatars per client; bypasses RLS via definer
create or replace function public.client_contributors()
returns table (
  client_id   uuid,
  client_name text,
  user_id     uuid,
  full_name   text,
  avatar_url  text
)
language sql
security definer
set search_path = public
stable
as $$
  select
    te.client_id,
    c.name as client_name,
    p.id as user_id,
    coalesce(p.full_name, 'Teammate') as full_name,
    p.avatar_url
  from public.time_entries te
  join public.clients c on c.id = te.client_id
  join public.profiles p on p.id = te.user_id
  where te.client_id is not null
  group by te.client_id, c.name, p.id, p.full_name, p.avatar_url;
$$;

-- Lock down public and grant to authenticated
revoke all on function public.client_contributors() from public;
grant execute on function public.client_contributors() to authenticated, service_role;

-- Refresh PostgREST schema
notify pgrst, 'reload schema';